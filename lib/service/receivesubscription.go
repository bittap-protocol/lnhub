...
func (svc *LndhubService) ConnectReceiveSubscription(ctx context.Context) (tapd.SubscribeReceiveAssetEventWrapper, error) {
	...
	return svc.TapdClient.SubscribeReceiveAssetEvent(ctx, &tapdevrpc.SubscribeReceiveAssetEventNtfnsRequest{})
}
...
func (svc *LndhubService) HandleTapdReceiveEvent(ctx context.Context, rcvEvent *tapdevrpc.ReceiveAssetEvent) (err error) {
	// check backoff first
	...
	if completeEvent != nil {
		...
		assetId := addressObj.AssetID
		// assetName := addressObj.Asset.AssetName
		svc.Logger.Infof("lnhub user found: %v", tahubUser.ID)
