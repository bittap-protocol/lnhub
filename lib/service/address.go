	...
	err = svc.DB.RunInTx(ctx, nil, func(ctx context.Context, tx bun.Tx) error {
		...
		if createAccounts {
			...
			for _, accountType := range accountTypes {
				// create account - TODO ensure joint uniqueness on type/asset_id
...
func (svc *LndhubService) FindAddress(ctx context.Context, userId uint64, taAssetId string, amt uint64) (*models.Address, error) {
	...
	err := svc.DB.NewSelect().Model(&addr).Where("user_id = ? AND amount = ? AND asset_id = ?", userId, amt, taAssetId).Limit(1).Scan(ctx)
	if err != nil {
...
func (svc *LndhubService) FindAddresses(ctx context.Context, userId uint64, taAssetId string) ([]models.Address, error) {
	addresses := []models.Address{}
	err := svc.DB.NewSelect().Model(&addresses).Where("user_id = ? AND asset_id = ?", userId, taAssetId).Scan(ctx)
	if err != nil {
		return nil, err
	}
