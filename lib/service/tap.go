...
func (svc *LndhubService) GetUniverseAssets(ctx context.Context) (okMsg string, success bool) {
	...
	for _, asset := range universeRoots {
		appendAsset := fmt.Sprintf("%s %s,", asset.AssetID, asset.AssetName)
...
func (svc *LndhubService) GetUniverseAssetsJson(ctx context.Context) (map[string]string, error) {
	...
	for _, asset := range assets {
		assetMap[asset.AssetID] = asset.AssetName
	}
	return assetMap, nil
}
	...
	if !hasFunding {
		...
		if err == nil && rcvAddr == nil {
			...
			entry := models.TransactionEntry{
				...
				AssetID:         sendAssetId,
				Outpoint:        models.LnhubInternalOutpoint,
				BroadcastState:  models.LnhubInternalComplete,
			}
			// insert the tx entry
